...
"""MuSHR Nano SUS 키보드 텔레오퍼레이션 스크립트 (UWRLL / mushr_nano_v2.usd).

WheeledLab MuSHR Nano v2 (서스펜션 추가 버전) 로봇을 키보드로 실시간 조종합니다.
4WD + Ackermann 전륜 조향 + 4륜 서스펜션 구조.
...
관절 구조:
    ...
    서스펜션 (passive, ImplicitActuator stiffness=1e8):
...
parser = argparse.ArgumentParser(description="MuSHR Nano SUS 키보드 텔레오퍼레이션")
...
SPAWN_HEIGHT = 0.12   # m (서스펜션 높이 고려, MuSHR보다 약간 높게)
...
class KeyboardController:
    ...
    def _print_help(self):
        print("\n" + "=" * 54)
        print("  MuSHR Nano SUS 키보드 텔레오퍼레이션")
        print("=" * 54)
        print("  W / ↑   전진       S / ↓   후진")
        print("  A / ←   좌회전    D / →   우회전")
        print("  SPACE   즉시 정지")
...
def design_scene() -> Articulation:
    ...
    robot_cfg = MUSHR_SUS_CFG.replace(
        prim_path="/World/MuSHR_SUS",
        init_state=MUSHR_SUS_CFG.init_state.replace(pos=(0.0, 0.0, SPAWN_HEIGHT)),
    )
...
def main():
    ...
    print(f"[텔레오프] 서스펜션 관절 포함 (passive, 제어 대상 아님)")
    print(f"[텔레오프] 관절 수: {len(jnames)}")
