...
"""F1TENTH 키보드 텔레오퍼레이션 스크립트.

WheeledLab F1TENTH 로봇을 키보드로 실시간 조종합니다.
4WD + Ackermann 전륜 조향 구조.

관절 구조:
    조향 (position 제어):
        rotator_left   — 좌 전륜 조향
        rotator_right  — 우 전륜 조향

    구동 (velocity 제어, DCMotor):
        wheel_back_left, wheel_back_right   — 후륜
        wheel_front_left, wheel_front_right — 전륜
...
사용 예시:
    /workspace/isaaclab/isaaclab.sh -p scripts/autodrive/teleop_f1tenth.py
"""
...
parser = argparse.ArgumentParser(description="F1TENTH 키보드 텔레오퍼레이션")
...
# ── F1TENTH 물리 파라미터 ──────────────────────────────────────────────────────
# F1TENTH 1/10 스케일 RC카, 바퀴 반지름 약 54mm
WHEEL_RADIUS    = 0.054   # m (추정값; USD 실측값으로 조정 가능)
...
# 관절 이름
STEER_LEFT_NAME   = "rotator_left"
STEER_RIGHT_NAME  = "rotator_right"
THROTTLE_NAMES    = [
    "wheel_back_left",
    "wheel_back_right",
    "wheel_front_left",
...
# 스폰 높이 (USD 루트 링크 기준; 바퀴 반지름보다 크게 설정)
SPAWN_HEIGHT = 0.1   # m
...
class KeyboardController:
    """carb.input 기반 범용 Ackermann 키보드 컨트롤러."""

    def __init__(self, max_speed: float = 3.0, max_steer: float = MAX_STEER):
    ...
    def _print_help(self):
        print("\n" + "=" * 54)
        print("  F1TENTH 키보드 텔레오퍼레이션")
        print("=" * 54)
        print("  W / ↑   전진       S / ↓   후진")
        print("  A / ←   좌회전    D / →   우회전")
        print("  SPACE   즉시 정지")
        print("  1       저속 1.0 m/s")
        print("  2       중속 3.0 m/s  [기본값]")
        print("  3       고속 5.0 m/s")
        print("  R       위치 초기화")
        print("  Q / ESC 종료")
        print("=" * 54 + "\n")
...
def design_scene() -> Articulation:
    ...
    robot_cfg = F1TENTH_CFG.replace(
        prim_path="/World/F1TENTH",
        init_state=F1TENTH_CFG.init_state.replace(pos=(0.0, 0.0, SPAWN_HEIGHT)),
    )
...
def main():
    ...
    while simulation_app.is_running():
        ...
        #       로봇이 제자리에서 회전하면 idx_throttles[1], [3] (right) 부호 반전
